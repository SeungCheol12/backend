...
public class MemberController {
    ...
    @GetMapping("/register")
    public void getMethodName(CustomUserDTO customUserDTO) {
        log.info("회원가입 폼 요청");
    }

    @PostMapping("/register")
    public String postMethodName(@Valid CustomUserDTO customUserDTO, BindingResult result) {
        ...
        try {
            ...
            Long mid = customUserService.join(customUserDTO);
            return "redirect:/member/login";
        } catch (Exception e) {
            log.info(e.getMessage());
